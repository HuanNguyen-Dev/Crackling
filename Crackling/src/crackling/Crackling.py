...
def Crackling(configMngr):
    ...
    for batchFile in guideBatchinator:
        ...
        if (configMngr['consensus'].getboolean('CHOPCHOP')):
            ...
            sectionTimers['G20'] = time.time() - start
        ############################################
        ...
        if (configMngr['consensus'].getboolean('mm10db')):
            ...
            printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
            sectionTimers['mm10db_Filters'] = time.time() - start
        #########################################
        ...
        if (configMngr['consensus'].getboolean('mm10db')):
            ...
            printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
            sectionTimers['AT_Filtering'] = time.time() - start
        ############################################
        ...
        if (configMngr['consensus'].getboolean('mm10db')):
            ...
            printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
            sectionTimers['Removing_TTTT_Targets'] = time.time() - start
        ##########################################
        ...
        if (configMngr['consensus'].getboolean('mm10db')):
            ...
            printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
            sectionTimers['RNAfold'] = time.time() - start
            if errorCount > 0:
        ...
        if (configMngr['consensus'].getboolean('mm10db')):
            ...
            sectionTimers['calc_mm10db'] = time.time() - start
            del acceptedCount
        ...
        if (configMngr['consensus'].getboolean('sgRNAScorer2')):
            ...
            printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
            sectionTimers['sgRNAScorer2'] = time.time() - start
        ...
        printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
        sectionTimers['efficacy_Consensus'] = time.time() - start
        if (configMngr['offtargetscore'].getboolean('enabled')):
            ...
            printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
            sectionTimers['Bowtie'] = time.time() - start
            #########################################
            ...
            for pgIdx, pageCandidateGuides in Paginator(
                ...
                printer(f'\t{failedCount:,} of {testedCount:,} failed here.')
                sectionTimers['OffTarget_Score'] = time.time() - start
        #########################################
        ##           Begin output              ##
        #########################################
        ...
        ##             Summary Print           ##
        #########################################
        printer('--- Section Breakdown ---')
        batchElapsed = time.time() - batchStartTime
        for section, duration in sectionTimers.items():
            # Calculate percentage of total batch time for better insight
            percent = (duration / batchElapsed) * 100
            printer(f'  {section:.<20} {duration:>8.2f}s ({percent:>5.1f}%)')
        printer('-------------------------')
